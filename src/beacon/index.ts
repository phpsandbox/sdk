...
export class Beacon implements BeaconActions {
   ...
   */
  private sendMessage<T = unknown>(type: string, payload: T = {} as T): void {
    if (!this.port) {
   ...
   */
  private async sendAndWaitFor<TPayload = unknown, TResponse = unknown>(
    sendType: string,
    responseType: string,
    ...
    return retry(
      async (bail) => {
        return new Promise<TResponse>((resolve, reject) => {
          ...
          const unsubscribe = this.eventEmitter.listen(responseType, (data: unknown) => {
            clearTimeout(timeoutId);
            unsubscribe.dispose();
   ...
   */
  public async executeCode(code: string): Promise<{ success: boolean; result?: unknown; error?: string }> {
   ...
   */
  public async inspectElement(selector: string): Promise<{ success: boolean; element?: unknown; error?: string }> {
   ...
   */
  public async captureScreenshot(request: ScreenshotRequest = {}): Promise<ScreenshotResult> {
    const mediaTimeoutMs = request.mediaTimeoutMs ?? request.timeout ?? 30000;
    const timeoutMs = request.transportTimeoutMs ?? mediaTimeoutMs + 5000;
...
export async function connectBeacon(
  iframe: HTMLIFrameElement,
  options?: BeaconOptions
): Promise<Beacon> {
  const beacon = new Beacon(iframe, options);
  try {
    await beacon.start();
  } catch (error) {
    beacon.dispose();
